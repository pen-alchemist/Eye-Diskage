...
class TestMainPage:
    ...
    def test_main_page_title(self, chrome_driver, wait_for):
    ...
    def test_main_page_url(self, chrome_driver, wait_for):
        main_page = page.MainPage(chrome_driver)
    ...
    def test_navigation_buttons(self, chrome_driver, wait_for, button_method, description):
        main_page = page.MainPage(chrome_driver)
        with allure.step(f"Click navigation button: {description}"):
            ...
            allure.attach(chrome_driver.current_url, "Redirected URL", allure.attachment_type.TEXT)
