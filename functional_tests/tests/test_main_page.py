...
class TestMainPage:
    ...
    def test_main_page_title(self, browser_driver, wait_for):
        main_page = page.MainPage(browser_driver)
        with allure.step("Verify the title matches the expected value"):
            ...
            allure.attach(browser_driver.title, "Page Title", allure.attachment_type.TEXT)

    @allure.story("URL Verification")
    @allure.title("Verify Main Page URL is Correct")
    def test_main_page_url(self, browser_driver, wait_for):
        main_page = page.MainPage(browser_driver)
        with allure.step("Verify the current URL matches expected"):
            ...
            allure.attach(browser_driver.current_url, "Current URL", allure.attachment_type.TEXT)

    @allure.story("Header Verification")
    @allure.title("Verify Main Page Header Matches Expected Text")
    def test_main_page_header(self, browser_driver, wait_for):
        main_page = page.MainPage(browser_driver)
    ...
    def test_navigation_buttons(self, browser_driver, wait_for, button_method, description):
        main_page = page.MainPage(browser_driver)
        with allure.step(f"Click navigation button: {description}"):
            ...
            allure.attach(browser_driver.current_url, "Redirected URL", allure.attachment_type.TEXT)
