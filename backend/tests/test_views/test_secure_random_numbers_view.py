...
def test_secure_random_numbers_view_post_unique_numbers(client_django):
    """Test that all numbers are unique when unique=True."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={'min_value': 1, 'max_value': 10, 'count': 5, 'unique': True})
    response_data = response.json()
    random_numbers = response_data['random_numbers']
    assert len(random_numbers) == len(set(random_numbers))


def test_secure_random_numbers_view_post_negative_values(client_django):
    """Test that negative values are handled correctly."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={'min_value': -10, 'max_value': -1, 'count': 5, 'unique': True})
    response_data = response.json()
    assert all(-10 <= num <= -1 for num in response_data['random_numbers'])


def test_secure_random_numbers_view_post_invalid_unique_type_status_code(client_django):
    """Test the status code for a POST request with an invalid unique type."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={'min_value': 1, 'max_value': 10, 'count': 5, 'unique': 'invalid'})
    assert response.status_code == status.HTTP_200_OK


def test_secure_random_numbers_view_post_empty_body_status_code(client_django):
    """Test the status code for a POST request with an empty body."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={})
    assert response.status_code == status.HTTP_400_BAD_REQUEST


def test_secure_random_numbers_view_post_empty_body_response_data(client_django):
    """Test the response data for a POST request with an empty body."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={})
    response_data = response.json()
    assert response_data == {"error": "min_value and max_value are required."}
...
def test_secure_random_numbers_view_post_invalid_data_status_code(client_django):
    """Test the status code for a POST request with invalid data."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={'min_value': 'invalid', 'max_value': 'invalid', 'count': 'invalid', 'unique': 'invalid'})
    assert response.status_code == status.HTTP_400_BAD_REQUEST


def test_secure_random_numbers_view_post_invalid_data_response_data(client_django):
    """Test the response data for a POST request with invalid data."""
    url = reverse('eye-secure-numbers')
    response = client_django.post(url, data={'min_value': 'invalid', 'max_value': 'invalid', 'count': 'invalid', 'unique': 'invalid'})
    response_data = response.json()
    assert "error" in response_data
